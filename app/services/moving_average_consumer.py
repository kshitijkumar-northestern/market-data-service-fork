...
class MovingAverageConsumer:
    def __init__(self):
        ...
        self.market_service = MarketService(None, self.kafka_service)  # DB will be injected per message
    ...
    async def process_price_event(self, message: Dict[str, Any]) -> None:
        ...
        try:
            ...
            recent_prices = db.query(PricePoint).filter(
                PricePoint.symbol == symbol
            ).order_by(desc(PricePoint.timestamp)).limit(5).all()
            ...
            prices = [p.price for p in reversed(recent_prices)]  # Reverse to get chronological order
